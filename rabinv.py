...
omega0 = gmr * B0 * 2 * pi
omega1 = gmr * B1 * 2 * pi
tlist = np.linspace(0,(1/4)*(pi/(np.sqrt(2)*omega1)),10000)
copconst = 0.03
...
NV.add_control(copconst*((sx*sy + sy*sx) + (sx**2 + sy**2)))
...
NV.pulses[1].coeff = np.array([-np.cos((omega0-D)*t) for t in tlist]) #for Rabi
cf2 = np.array([np.sin((omega0-D)*t) for t in tlist[:5000]])
cf2 = np.append(cf2,np.zeros(5000))
NV.pulses[2].coeff = np.array([np.sin((omega0-D)*t) for t in tlist]) #for Rabi
NV.pulses[3].coeff = np.array([t for t in tlist])
NV.pulses[0].tlist = tlist
NV.pulses[1].tlist = tlist
NV.pulses[2].tlist = tlist
NV.pulses[3].tlist = tlist 
